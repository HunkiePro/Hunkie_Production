...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['bookedDate']) && isset($_POST['bookedTime']) && isset($_POST['location'])) {
        // Sanitize and validate form data
        $bookedDate = htmlspecialchars(trim($_POST['bookedDate']));
        $bookedTime = htmlspecialchars(trim($_POST['bookedTime']));
        $location = htmlspecialchars(trim($_POST['location']));

        // Insert data into the database
        $stmt = $conn->prepare("INSERT INTO bookings (bookedDate, bookedTime, location) VALUES (?, ?, ?)");

        // Check if the prepare statement is successful
        if ($stmt) {
            $stmt->bind_param("sss", $bookedDate, $bookedTime, $location);
                ...
                <main>
                    <div class="container-fluid px-4">
                        <form style="margin-top: 40px;" action="" method="post">
                            ...
                            <input type="time" id="bookedTime" name="bookedTime" required>
